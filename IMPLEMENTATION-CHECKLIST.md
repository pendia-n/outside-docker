...
- [x] Give Verifiers a Web-only invitation, purchase, and review journey; comparison remains pending.
- [x] Use a light-only responsive layout for 13-inch desktop, iPad Air, mobile, and narrow folding phones.
...
- [ ] Support registration, login, logout, session, recovery, and reset through APIs and UI (recovery/reset remain pending).
...
- [x] Verify Supplier and Verifier registration/login with local test accounts only.

## Evidence and access model

- [x] Separate Event Type, Event Instance, and append-only Event Record in D1 and owner-authorized APIs.
...
- [x] Make Track M API accept structured strings or local-file bytes programmatically, hash ephemerally, and return content hash, commitment, manifest, and signed receipt without retaining originals.
- [x] Preserve API-key scopes, owner authorization, idempotency, rate limits, and append ordering.
...
- [x] Apply all D1 migrations locally and verify schema invariants.
- [x] Pass typecheck, unit/integration tests, production build, and contract compilation.
- [x] Verify registration/login and role boundaries through both UI and API.
- [x] Verify Track H UI/API and Track M API with text and local-document fixtures.
